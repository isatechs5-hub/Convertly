...
import { ArrowLeft, Download, Loader2, Trash2, Plus, FileText } from 'lucide-react';
import { AppView } from '../types';
import { removePagesFromPdf } from '../services/pdfUtils';

interface Props {
  setView: (view: AppView) => void;
}

export const RemovePages: React.FC<Props> = ({ setView }) => {
        ...
        parts.forEach(part => {
            if (part.includes('-')) {
                ...
                if (!isNaN(start) && !isNaN(end)) {
                    for (let i = start; i <= end; i++) indicesToRemove.push(i - 1);
                }
            } else {
                const num = Number(part);
                if (!isNaN(num)) indicesToRemove.push(num - 1);
            }
        });

        // Deduplicate
        const uniqueIndices = [...new Set(indicesToRemove)].filter(i => i >= 0);

        const bytes = await removePagesFromPdf(file, uniqueIndices);
      ...
      <div className="max-w-3xl mx-auto px-4">
        <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="p-8 text-center border-b border-slate-100 bg-slate-50/50">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Remove Pages</h2>
            <p className="text-slate-500 max-w-md mx-auto">Delete specific pages from your PDF document.</p>
          </div>
          ...
          <div className="p-8">
             {!file ? (
                ...
                <div className="mb-8">
                    ...
                    <label className="block text-sm font-medium text-slate-700 mb-2">Pages to Remove</label>
                    <input 
                        type="text" 
                    ...
                    />
                    <p className="text-xs text-slate-500 mt-2">Enter page numbers to delete, separated by commas or ranges.</p>
                </div>
             )}

            <button
              onClick={handleProcess}
              disabled={!file || !pagesToRemove || isProcessing}
              className={`w-full py-4 rounded-xl font-bold text-lg text-white shadow-lg flex items-center justify-center gap-2 transition-all ${!file || !pagesToRemove ? 'bg-slate-300 cursor-not-allowed' : 'bg-brand-600 hover:bg-brand-700 hover:scale-[1.02]'}`}
            >
              {isProcessing ? <Loader2 className="animate-spin h-6 w-6" /> : <Trash2 className="h-6 w-6" />}
