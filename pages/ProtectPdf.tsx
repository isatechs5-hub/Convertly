...
import { ArrowLeft, Download, Loader2, Lock, Plus, Trash2 } from 'lucide-react';
import { AppView } from '../types';
import { protectPdfDocument } from '../services/pdfUtils';

interface Props {
  setView: (view: AppView) => void;
}

export const ProtectPdf: React.FC<Props> = ({ setView }) => {
  ...
  const handleProtect = async () => {
    ...
    try {
      const bytes = await protectPdfDocument(file, password);
      ...
      <div className="max-w-3xl mx-auto px-4">
        <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="p-8 text-center border-b border-slate-100 bg-slate-50/50">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Protect PDF</h2>
            <p className="text-slate-500 max-w-md mx-auto">Encrypt your PDF with a password.</p>
          </div>
          ...
          <div className="p-8">
            ...
            >
              {isProcessing ? <Loader2 className="animate-spin h-6 w-6" /> : <Lock className="h-6 w-6" />}
