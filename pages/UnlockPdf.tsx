...
import { ArrowLeft, Download, Loader2, Unlock, Plus, Trash2 } from 'lucide-react';
import { AppView } from '../types';
import { unlockPdfDocument } from '../services/pdfUtils';

interface Props {
  setView: (view: AppView) => void;
}

export const UnlockPdf: React.FC<Props> = ({ setView }) => {
  ...
  const handleUnlock = async () => {
    ...
    try {
      const bytes = await unlockPdfDocument(file, password);
      ...
      console.error(error);
      alert("Failed to unlock PDF. Incorrect password or corrupt file.");
    } finally {
      ...
      <div className="max-w-3xl mx-auto px-4">
        <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="p-8 text-center border-b border-slate-100 bg-slate-50/50">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Unlock PDF</h2>
            <p className="text-slate-500 max-w-md mx-auto">Remove password security from your PDF.</p>
          </div>
          ...
          <div className="p-8">
            ...
            <button
              ...
              className={`w-full py-4 rounded-xl font-bold text-lg text-white shadow-lg flex items-center justify-center gap-2 transition-all ${!file || !password ? 'bg-slate-300 cursor-not-allowed' : 'bg-brand-600 hover:bg-brand-700 hover:scale-[1.02]'}`}
            >
              {isProcessing ? <Loader2 className="animate-spin h-6 w-6" /> : <Unlock className="h-6 w-6" />}
